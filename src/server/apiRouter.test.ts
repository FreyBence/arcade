...
function createApi(): ApplicationApi {
  ...
  return { register: handler, login: handler, refresh: handler, logout: handler, identity: handler, updateProfile: handler, changePassword: handler, googleAuthorization: handler, googleCallback: handler, adminUsers: handler, updateAdminDinoCoins: handler, updateAdminRole: handler }
}
...
const routeCases = [
  ...
  { name: 'routes admin Dino Coin updates', input: { path: '/api/admin/users/dino-coins', method: 'PATCH' }, expected: { status: 200, allow: null, code: undefined } },
  { name: 'routes admin role updates', input: { path: '/api/admin/users/role', method: 'PATCH' }, expected: { status: 200, allow: null, code: undefined } },
